...
int class_system::read_input(int&   Nneurons_per_layer,
  ...
  if (!file) {
    perror("System read input error: opening file error");                
    return 1;
  }
  ...
  int new_calc_read              = 1;
  int steps_to_initialize_read   = 1;
  int initial_reduction_eta_read = 1;        
  ...
  while (fgets(line, sizeof(line), file)) {
    ...
    else if (strcmp(var, "new_calculation") == 0) {
      new_calc                       = val1;
      new_calc_read                  = 0;      
    }
    else if (strcmp(var, "steps_to_initialize") == 0) {
      steps_to_initialize            = val1;
      steps_to_initialize_read       = 0;      
    }
    else if (strcmp(var, "initial_reduction_eta") == 0) {
      initial_reduction_eta          = val1;
      initial_reduction_eta_read     = 0;      
    }                                              
  }
  ...
  if (new_calc_read == 1)  {
    printf("system read input error: new_calculation was not read\n");
    return 1;
  }
  if (steps_to_initialize_read == 1)  {
    printf("system read input error: steps_to_initialize was not read\n");
    return 1;
  }
  if (initial_reduction_eta_read == 1)  {
    printf("system read input error: initial_reduction_eta was not read\n");
    return 1;
  }                      
