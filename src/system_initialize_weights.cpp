...
int class_system::initialize_weights() {
  ...
  //   Weights W must be initially positive
  for (int i = 0; i < 2 * Nneurons; i++) {
    W_hidden[0][i] = std::abs(dist(gen));
    V_hidden[0][i] = dist(gen);
  }
  ...
  for (int k = 1; k < Nhidden; k++) 
    for (int i = 0; i < Nneurons * Nneurons; i++)    
      W_hidden[k][i] = std::abs(dist(gen));
  for (int k = 1; k < Nhidden; k++) 
    for (int i = 0; i < 2 * Nneurons; i++)    
      V_hidden[k][i] = dist(gen);  
  for (int k = 1; k < Nhidden; k++) 
    for (int i = 0; i < Nneurons; i++)    
      b_hidden[k][i] = dist(gen);
  //-- Weights from last hidden layer to output (1 output) --
  for (int i = 0; i < Nneurons; i++)
    W_out[i] = std::abs(dist(gen));
  b_out[0] = dist(gen);  // One output
  V_out[0] = dist(gen);  // Two inputs
  V_out[1] = dist(gen);

  // Offset of each file
  //i == 0 is initialized as zero, and does not change
  phi[0] = 0;
