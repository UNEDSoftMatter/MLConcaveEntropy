...
void class_system::print_approximation_function(int    step,
  ...
  fprintf(file, "sigma(x) = -log(1.0 + exp(-x))\n");
  ...
  cudaMemcpy(temp, k_V_hidden, Nhidden * sizeof(real*), cudaMemcpyDeviceToHost);
  for (int k = 0; k < Nhidden; k++) {
    int size_k;
    size_k = Nneurons * 2;
    cudaMemcpy(V_hidden[k],      // destino CPU
               temp[k],           // origen GPU
               size_k * sizeof(real),
               cudaMemcpyDeviceToHost);
  }
  delete[] temp;
  // Weights V are written
  for (int k = 0; k < Nhidden; k++)
    for (int i = 0; i < 2; i++) // 2 inputs
      for (int j = 0; j < Nneurons; j++) {
	n = i*Nneurons + j;
	fprintf(file, "V_%d_%d_%d = " REAL_FMT "\n", j, i, k, V_hidden[k][n]);
      }
  ...
  //---------------------
  // Weights from the last hidden layer to the output layer
  cudaMemcpy(this->V_out,  k_V_out,  2 * sizeof(real), cudaMemcpyDeviceToHost);
  k_aux = Nhidden;
  j_aux = 0;
  for (int i = 0; i < 2; i++)
    fprintf(file, "V_%d_%d_%d = " REAL_FMT "\n", j_aux, i, k_aux, V_out[i]);  
  ...
  //---------------------------
  //--- OFFSET PARAMETER PHI --
  //--------------------------
  cudaMemcpy(this->phi,  k_phi, Nfiles * sizeof(real), cudaMemcpyDeviceToHost);
  for (int k = 0; k < Nfiles; k++)  
    fprintf(file, "phi_%d = " REAL_FMT "\n", k, phi[k]);
  ...
  for (int j = 0; j < Nneurons; j++) {
	...
	fprintf(file, "+ W_%d_%d_%d * z_in_%d(x_0, x_1) +"
                      " + V_%d_%d_%d * z_in_%d(x_0, x_1) \\\n",
		       j, i, k_aux, i, j, i, k_aux, i);
      else
	fprintf(file, "+ W_%d_%d_%d * z_in_%d(x_0, x_1) + "
                       "  V_%d_%d_%d * z_in_%d(x_0, x_1) + b_%d_%d \n",
		         j, i, k_aux, i, j, i, k_aux, i, j, k_aux);
  ...
  for (int k = 1; k < Nhidden; k++)  
	...
	else // if (i == Nneurons + 1)
	  fprintf(file, "+ W_%d_%d_%d * z_%d_%d(x_0, x_1) +" 
                        "   V_%d_0_%d * z_in_0(x_0, x_1) + "
                        "   V_%d_1_%d * z_in_1(x_0, x_1) + b_%d_%d \n",
            		   j, i, k, i, k-1, j, k, j, k, j, k);
      fprintf(file, "z_%d_%d(x_0, x_1) = sigma(a_%d_%d(x_0, x_1)) \n\n", j, k, j, k);
}
  ...
  for (int i = 0; i < Nneurons; i++) 
    if (i < Nneurons - 1)
      ...
      fprintf(file, "+ W_%d_%d_%d * z_%d_%d(x_0, x_1) + "
                    "   V_%d_0_%d * z_in_0(x_0, x_1) + "
	            "   V_%d_1_%d * z_in_1(x_0, x_1) + b_%d_%d \n",
	            0, i, Nhidden, i, Nhidden-1, 0, Nhidden, 0, Nhidden, 0, Nhidden);

  //-- The order to plot is written --
  fprintf(file, "splot f(x,y), \\\n");
  for (int k = 0; k < Nfiles-1; k++) 
    fprintf(file, "'%s' u 1:2:($3 - phi_%d), \\\n", data_file_names[k].c_str(), k);
  fprintf(file, "'%s' u 1:2:($3 - phi_%d) \n", data_file_names[Nfiles - 1].c_str(), Nfiles-1);
