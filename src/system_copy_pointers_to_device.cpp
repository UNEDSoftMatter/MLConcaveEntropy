...
void class_system::copy_pointers_to_device(real**  k_data,
  ...
  //--- file_end ---
  cudaMalloc(k_file_end, Nfiles * sizeof(int));
  cudaMemcpy(*k_file_end, file_end, Nfiles * sizeof(int), cudaMemcpyHostToDevice);  
  ...
  //--- V_hidden ---
  cudaMalloc(k_V_hidden, Nhidden * sizeof(real*));
  ...
  for (int k = 0; k < Nhidden; k++) {
    ...
    // We reserve memory in GPU for the k layer
    real* dVk;
    cudaMalloc(&dVk, size_k * sizeof(real));
    // The layer is copied from the CPU to the GPU
    cudaMemcpy(dVk, V_hidden[k], size_k * sizeof(real), cudaMemcpyHostToDevice);    
    // The pointer of device of this layer is saved
    temp[k] = dVk;
  }
  // The array of pointers is copied to the device
  cudaMemcpy(*k_V_hidden, temp, Nhidden * sizeof(real*), cudaMemcpyHostToDevice);
  // The temporal array is freed
  delete[] temp;

  //--- V_out ---
  cudaMalloc(k_V_out, 2 * sizeof(real)); // 1 output, 2 inputs
  cudaMemcpy(*k_V_out, V_out, 2 * sizeof(real), cudaMemcpyHostToDevice);    
  ...
  //--- phi ---
  cudaMalloc(k_phi, Nfiles * sizeof(real)); 
  cudaMemcpy(*k_phi, phi, Nfiles * sizeof(real), cudaMemcpyHostToDevice);  
  ...
  //--- batch_N ----
  cudaMalloc(k_batch_N, Nbatches * sizeof(int));
  cudaMemcpy(*k_batch_N, batch_N, Nbatches * sizeof(int), cudaMemcpyHostToDevice);

  //--- batch_index ----
  cudaMalloc(k_batch_file, Nbatches * sizeof(int));
  cudaMemcpy(*k_batch_file, batch_file, Nbatches * sizeof(int), cudaMemcpyHostToDevice);
  ...
  //--- grad_V_hidden ---
  cudaMalloc(k_grad_V_hidden, Nhidden * sizeof(real*));
  temp = new real*[Nhidden];  // Temporal array in CPU containing pointers of device
  for (int k = 0; k < Nhidden; k++) {
    int size_k;
    size_k = Nneurons * 2;
    // We reserve memory in GPU for the k layer
    real* d_grad_Vk;
    cudaMalloc(&d_grad_Vk, size_k * sizeof(real));
    // The layer is copied from the CPU to the GPU
    cudaMemset(d_grad_Vk, 0, size_k * sizeof(real));        
    // The pointer of device of this layer is saved
    temp[k] = d_grad_Vk;
  }
  // The array of pointers is copied to the device
  cudaMemcpy(*k_grad_V_hidden, temp, Nhidden * sizeof(real*), cudaMemcpyHostToDevice);
  // The temporal array is freed
  delete[] temp;  
  ...
  //--- grad_V_out ---
  cudaMalloc(k_grad_V_out, 2 * sizeof(real)); // 2 inputs
  cudaMemset(*k_grad_V_out, 0, 2 * sizeof(real));  
  ...
  //--- grad_phi ---
  cudaMalloc(k_grad_phi, Nfiles * sizeof(real)); // One per each data set (file)
  cudaMemset(*k_grad_phi, 0, Nfiles * sizeof(real));    
  ...
  //--- m_V_hidden ---
  cudaMalloc(k_m_V_hidden, Nhidden * sizeof(real*));
  temp = new real*[Nhidden];  // Temporal array in CPU containing pointers of device
  for (int k = 0; k < Nhidden; k++) {
    int size_k;
    size_k = Nneurons * 2;
    // We reserve memory in GPU for the k layer
    real* d_m_Vk;
    cudaMalloc(&d_m_Vk, size_k * sizeof(real));
    // The layer is copied from the CPU to the GPU
    cudaMemset(d_m_Vk, 0, size_k * sizeof(real));        
    // The pointer of device of this layer is saved
    temp[k] = d_m_Vk;
  }
  // The array of pointers is copied to the device
  cudaMemcpy(*k_m_V_hidden, temp, Nhidden * sizeof(real*), cudaMemcpyHostToDevice);
  // The temporal array is freed
  delete[] temp;

  //--- m_V_out ---
  cudaMalloc(k_m_V_out, 2 * sizeof(real)); // 2 inputs
  cudaMemset(*k_m_V_out, 0, 2 * sizeof(real));
  
  //--- v_V_hidden ---
  cudaMalloc(k_v_V_hidden, Nhidden * sizeof(real*));
  temp = new real*[Nhidden];  // Temporal array in CPU containing pointers of device
  for (int k = 0; k < Nhidden; k++) {
    int size_k;
    size_k = Nneurons * 2;
    // We reserve memory in GPU for the k layer
    real* d_v_Vk;
    cudaMalloc(&d_v_Vk, size_k * sizeof(real));
    // The layer is copied from the CPU to the GPU
    cudaMemset(d_v_Vk, 0, size_k * sizeof(real));        
    // The pointer of device of this layer is saved
    temp[k] = d_v_Vk;
  }
  // The array of pointers is copied to the device
  cudaMemcpy(*k_v_V_hidden, temp, Nhidden * sizeof(real*), cudaMemcpyHostToDevice);
  // The temporal array is freed
  delete[] temp;

  //--- v_V_out ---
  cudaMalloc(k_v_V_out, 2 * sizeof(real)); // 2 inputs
  cudaMemset(*k_v_V_out, 0, 2 * sizeof(real));    
  ...
  //--- m_phi ---
  cudaMalloc(k_m_phi, Nfiles * sizeof(real)); // One per each data set (file)
  cudaMemset(*k_m_phi, 0, Nfiles * sizeof(real));  
  ...
  //--- v_phi ---
  cudaMalloc(k_v_phi, Nfiles * sizeof(real)); // One per each data set (file)
  cudaMemset(*k_v_phi, 0, Nfiles * sizeof(real));  
